...
public class JNetworkplan extends JPanel implements MouseListener, MouseMotionListener {
  ...
  /**
   * Initialize the position-Matrix and tupel-Array for each row. Add each
   * networkplan element on this JPanel.
   *
  ...
  public void paintJNetworkplanElements() {
    ...
    int child[] = npElemente.get(0).getChild();
    ...
    while (middleElementsIt.hasNext()) {
      ...
      /**
       * If this element is a long vertical connection line checkIfLine is
       * negative
       */
      ...
      NetworkplanElement np = npElemente.get(currentElementNumber - 1);
      ...
      if (checkIfLine > 0) {
        ...
        elementsPosition.put(np.getIndex(), new Point(jNPaddingX + x * jNpElemBoundsX, jNPaddingY + middlePos * jNpElemBoundsY));
        i++;
      }
      x++;
    }
    ...
    while (topPos >= 0 && !onlyOneElement) {
      ...
      while (topElementsIt.hasNext()) {
        int currentElementNumber = ((Integer) topElementsIt.next());
    ...
    while (tupel[bottomPos] != null && !onlyOneElement) {
      ...
      while (bottomElementsIt.hasNext()) {
        ...
        if (checkIfLine > 0) {
           ...
           */
          /**
           * Check if child element has only one parent and vice versa. If true
           * the element's position is right under the position of his parent.
           * This position is get with the getAnchor()-Method.
           */
          int parent[] = np.getParent();
          if (parent.length == 1) {
            child = npElemente.get(parent[0] - 1).getChild();
            if (child.length == 1) {
              int anchorX = (int) jNpElem[npElemente.get(parent[0] - 1).getLayoutManager()].getAnchorPoint(JNetworkplanElement.ANCHOR_BOTTOM_MIDDLE).getX() - jNpElemWidth / 2;
              int anchorY = (int) jNpElem[npElemente.get(parent[0] - 1).getLayoutManager()].getAnchorPoint(JNetworkplanElement.ANCHOR_BOTTOM_MIDDLE).getY() + 110;

              jNpElem[i].setBounds(anchorX, anchorY, jNpElemWidth, jNpElemHeight);
              elementsPosition.put(np.getIndex(), new Point(anchorX, anchorY));
            }
          } else {
            Hashtable childWidth = getRelWidthOfChilds(bottomPos);
            double freeWidthProzent = ((Double) childWidth.get(np.getIndex()));
            ...
            elementsPosition.put(np.getIndex(), new Point(jNPaddingX + y * jNpElemBoundsX + freeWidth, jNPaddingY + bottomPos * jNpElemBoundsY));
          }
          i++;
        }
        y++;
   ...
   * position is a coordinate system with x and y axis. Each x/y-coordinate can
   * contain a number of a network plan element. This is used to get the row of
   * the networkplan with the biggest width.
   */
  public void calculatePositions() {
     ...
     */
    tupel[0] = new Vector();
    tupel[0].add(npElemente.get(0).getIndex());
    ...
    while (!lastElement) {
      ...
      while (a < tupel[i].size()) {
        ...
        child = npElemente.get(currentElem - 1).getChild();
        ...
        while (b < child.length) {
          ...
          int parent[] = npElemente.get(child[b] - 1).getParent();

          /**
           * Add this element because the parent exists
           */
          if (parent.length == 1) {
            tupel[i + 1].add(child[b]);
          } /**
           ...
           */
            ...
            while (c < parent.length) {
               ...
               */
              if (!tupel[i].contains(parent[c]) && !tupel[i].contains(parent[c] * (-1))) {
                parentsExists = false;
                break;
              }
              c++;
            ...
            if (parentsExists) {
               ...
               */
              if (!tupel[i + 1].contains(child[b])) {
                tupel[i + 1].add(child[b]);
              }

              /**
               * Add the negative value of the parent again, because some
               * parents for this child dont exist
               */
            } else {
              tupel[i + 1].add(npElemente.get(currentElem - 1).getIndex() * (-1));
            }
          }
          b++;
        }
    ...
    while (j < i) {
      ...
      while (actTupelIt.hasNext()) {
        ...
        if (actInt > 0) {
          NetworkplanElement actNpElem = npElemente.get(actInt - 1);
  ...
  public Hashtable getRelWidthOfChilds(int tupelNr) {
    ...
    if (tupel[tupelNr].size() == 1) {
      childWidth.put(tupel[tupelNr].get(0), 100.0);
      return childWidth;
    }
    ...
    while (a < childs.size()) {
      ...
      if (tempPos > 0) {
        int parent[] = npElemente.get(currentPos - 1).getParent();
        int b = 0;
        while (b < parent.length) {
          if (!parents.contains(parent[b])) {
            parents.add(parent[b]);
          }
          b++;
        }
      } else {
        if (!parents.contains((tempPos))) {
          parents.add(tempPos);
        }
      }
    ...
    while (c < parents.size()) {
      ...
      if (tempPos > 0) {
        int child[] = npElemente.get(currentPos - 1).getChild();

        int countChilds = child.length;

        double widthForChild = widthForChildGroup / (double) countChilds;

        int d = 0;
        while (d < child.length) {
          if (childWidth.containsKey(child[d])) {
            double temp = ((Double) childWidth.get(child[d]));
            childWidth.put(child[d], widthForChild + temp);
          } else {
            childWidth.put(child[d], widthForChild);
          }
          d++;
        }
      } else {
        double widthForChild = widthForChildGroup;
        childWidth.put(tempPos * (-1), widthForChild);
      }
      c++;
    }
    return childWidth;
  ...
  public void mouseReleased(MouseEvent evt) {
    if (movedElement != null) {
       ...
       */
      elementsPosition.put(movedElement.getNp().getIndex(), movedElement.getLocation());
    ...
    if (getComponentAt(x, y).getName() != null && !editPanelOpen) {
      ...
      jButtonEdit.addActionListener(new java.awt.event.ActionListener() {
        public void actionPerformed(java.awt.event.ActionEvent evt) {
          ...
          /**
           * Check if the number is not a string. If its a string reset to old
           * number
           */
          try {
          ...
          npElemente.get(clickedComponent.getNp().getIndex() - 1).setDuration(Double.parseDouble(jTextFieldDuration.getText()));
          npElemente.get(clickedComponent.getNp().getIndex() - 1).setDescription(jTextFieldDescription.getText());
          ...
          /**
           * Set positions of all connection lines between the
           * JNetworkplanElements
           */
      ...
      jPanelEdit.add(jButtonCancel);
      jButtonCancel.setBounds(145, 82, 81, 26);
      jButtonCancel.setBorder(new LineBorder(Color.BLACK));
  ...
  public void resetCriticalPath() {
    ...
    while (a < npElemente.size()) {
      npElemente.get(a).setCriticalPath(false);
      a++;
    }
  }
  ...
  public void positionSavedElements() {
    ...
    for (int b = 0; b < jNpElem.length && jNpElem[b] != null; b++) {
      if (!jNpElem[b].isDisplayArrow()) {
        disabledArrowElements.add(jNpElem[b].getNp().getNumber());
      }
    }
    ...
    while (a < npElemente.size()) {
       ...
       */
      if (disabledArrowElements.contains(jNpElem[a].getNp().getNumber())) {
        jNpElem[a].setDisplayArrow(false);
      }

      this.add(jNpElem[a]);

      Point location = (Point) elementsPosition.get(np.getIndex());
  ...
  public void paintConnectionLines() {
    ...
    while (a < jNpElem.length && jNpElem[a] != null) {
      ...
      if (!jNpElem[a].getNp().isStartElem()) {
         ...
         * This Element is a pseudo Activity
         */
        if (jNpElem[a].getNp().isPseudoActivity() || npElemente.get(parent[0] - 1).isPseudoActivity()) {
      ...
      if (!jNpElem[a].getNp().isEndElem()) {
         ...
         * This Element is a pseudo Activity
         */
        if (jNpElem[a].getNp().isPseudoActivity() || npElemente.get(child[0] - 1).isPseudoActivity()) {
          horizontalCon[a] = new JDottedLine();
        }
   ...
   * JNetworkplanElement with the mouse. The connected lines to this element
   * move in dependence of the movement of the JNetworkplanElement.
   *
  ...
  public void positionConnectionLines(JNetworkplanElement jNpActElem) {
    ...
    /**
     * The moved JNetworkplanElement is not the End-Element and has a
     * connection-line on the bottom
     */
    ...
    /**
     * The moved JNetworkplanElement is not the Start-Element and has a
     * connection-line on the top
     */
  ...
  /**
   * This element has a vertical connection line on the bottom. Set its
   * position!
   *
  ...
  public void updateVerticalLinesBottom(JNetworkplanElement jNpActElem) {
    ...
    JNetworkplanElement childJNpElem = jNpElem[npElemente.get(child[0] - 1).getLayoutManager()];
    ...
    if (child.length == 1 && childsParents.length == 1) {
      ...
      JNetworkplanElement parentJNpElem = jNpElem[npElemente.get(parent[0] - 1).getLayoutManager()];

      /**
       * Set the position and size of the connection line
       */
      /**
       * YPosition of the ending point of the vertical connection line on bottom
      ...
      if (parent[0] != 0) {
        for (int u = 0; u < parent.length; u++) {
          updateHorizontalLinesBottom(jNpElem[npElemente.get(parent[u] - 1).getLayoutManager()]);
        }
      }
    ...
    if (child[0] != 0) {
      for (int u = 0; u < child.length; u++) {
        updateHorizontalLinesTop(jNpElem[npElemente.get(child[u] - 1).getLayoutManager()]);
      }
    }
  }
  ...
  public void updateHorizontalLinesTop(JNetworkplanElement jNpActElem) {
    ...
    if (parent[0] != 0) {
      ...
      if (difX != 0) {
        ...
        /**
         * YPosition of the ending point of the vertical connection line on
  ...
  public void updateCriticalPathLines() {
    ...
    while (keys.hasMoreElements()) {
      ...
      if (key.matches("^[0-9]*top$")) {
         ...
         */
        if (npElemente.get(Integer.parseInt(index) - 1).isCriticalPath()) {
          actSeparator.setForeground(Color.RED);
        }

        /**
         * This JSeperator is a vertical connection line on the bottom of an
         * element
         */
        ...
        /**
         * Check if the elements connected to this line is are critical path
         * member
         */
        if (npElemente.get(Integer.parseInt(index[0]) - 1).isCriticalPath() && npElemente.get(Integer.parseInt(index[1]) - 1).isCriticalPath()) {
          actSeparator.setForeground(Color.RED);
        }
      }
    }
  ...
  /**
   * Sync this elements vertical connection line in the top to the child
   * element's line
   *
   * @param jNpActElem The current JNetworkplanElement
   */
  ...
  /**
   * Sync this elements vertical connection line in the top to the parent
   * element's line
   *
   * @param jNpActElem The current JNetworkplanElement
   */
