...
public class NewWiSimGUI extends javax.swing.JFrame {
  ...
  private MDIDesktopPane mdiDesktop;
  ...
  private void initApplication() {
    ...
    setLocation(0, 0);
    mdiDesktop = new MDIDesktopPane();
    jScrollPane1.getViewport().add(mdiDesktop);
  ...
  private void addInternalFrame(String iFrame) {
    ...
    /* The internal frame is in the background or minimized */
    ...
    /* The internal frame wasn't open yet */  
    } else {
      newIFrame.setVisible(true);
      mdiDesktop.add(newIFrame);
      if (newIFrame.isPreferredSizeSet()) {
        newIFrame.setSize(newIFrame.getPreferredSize());
        
        if (newIFrame.getSize().width + lastIFramePostion.x + 25 > mdiDesktop.getSize().width || newIFrame.getSize().height + lastIFramePostion.y + 25 > mdiDesktop.getSize().height) {
          lastIFramePostion = new Point(-25, -25);
        }
        
        lastIFramePostion = new Point(lastIFramePostion.x + 25, lastIFramePostion.y + 25);
        if (newIFrame.getSize().width < mdiDesktop.getSize().width && newIFrame.getSize().height < mdiDesktop.getSize().height) {
          newIFrame.setLocation(lastIFramePostion.x, lastIFramePostion.y);
        } else {
          newIFrame.setSize(mdiDesktop.getSize());
          try {
            newIFrame.setMaximum(true);
          } catch (PropertyVetoException e) {
            System.err.println(e.getMessage());
