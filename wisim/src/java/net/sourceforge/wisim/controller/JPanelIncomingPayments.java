...
/**
 * JPanelZahlungseingang ermöglicht Übersicht und Bearbeitung aller
 * Zahlungseingänge.
 */
public class JPanelIncomingPayments extends javax.swing.JPanel implements Refreshable {

  private WiSimDAO dao;
  private final HashMap<String, Contract> vertragObjekte;
  private final HashMap<String, ContractInvoice> rechnungObjekte;
  private final HashMap<String, Customer> kundeObjekte;
  private int anzahl;
  ...
  public JPanelIncomingPayments(WiSimMainController wiSimMainController) {
    ...
    vertragObjekte = new HashMap<>();
    rechnungObjekte = new HashMap<>();
    kundeObjekte = new HashMap<>();
    anzahl = 0;
    isBuilt = false;
    initComponents();
   ...
   */
	private void initComponents() {//GEN-BEGIN:initComponents
		...
		jTableRechnungListe.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {
		}, new String[] { "Nr", "Name, Vorname", "Lieferung", "Zahlung" }) {
  ...
  public void loadInvoices() {
    try {
      Collection<Contract> vertraege = dao.getContracts();
      ...
      for (Contract vertragsliste : vertraege) {

        //while (it_Vertrag.hasNext()) {
        //Contract vertragsliste = (Contract) it_Vertrag.next();
        ContractInvoice auftrag = dao.getAuftragsrechnung(vertragsliste.getAuftragsrechnungsId());
        Customer kunde = dao.getCustomer(vertragsliste.getKundenId());
  ...
  private void loadCustomer() {
    ...
    Contract auswahlVertrag = vertragObjekte.get(listItem);
    ContractInvoice auswahlRechnung = rechnungObjekte.get(listItem);
    Customer auswahlKunde = kundeObjekte.get(listItem);
  ...
  private int getSelectedContractID() {
    ...
    Contract auswahl = vertragObjekte.get(listItem);
    if (auswahl != null) {
  ...
  /**
   * Wurde das Pane schon einmal aufgebaut, so ist "isBuilt" = TRUE. Wichtig ist
   * diese Variable für die Simulation: Wurde das Pane schon einmal aufgebaut,
   * so müssen die Simulationsthread das Pane nicht noch einmal initialisieren!
   *
   * @return boolean
   */
