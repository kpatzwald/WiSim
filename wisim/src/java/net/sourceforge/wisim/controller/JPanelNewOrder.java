...
/** Masken f�r die WiSimComponent-Bestellung
...
public class JPanelNewOrder extends javax.swing.JPanel {
	...
	/** �berpr�ft alle Pflichtfelder auf Inhalt. Gibt eine Fehlemeldung
	 * an den Benutzer, falls ein oder mehrere Felder leer gelassen
	 * wurden.
	 * @return false: Es wurden nicht alle Felder gef�llt.
	 * true: Alle Pflichtfelder sind gef�llt.
	 */
	private boolean checkAllFields() {
		...
		/*Wurde ein oder mehrere Pflichtfelder nicht ausgef�llt erscheint ein JOptionPane, dass dem
		 *Benutzer die fehlenden noch leeren Felder anzeigt*/
		if (!pflichtfelder.isEmpty()) {
			if (pflichtfelder.size() > 1)
				JOptionPane.showMessageDialog(this, "Folgende Felder m�ssen ausgef�llt werden: " + pflichtfelder.toString().substring(1, pflichtfelder.toString().length() - 1), "Fehler beim Anlegen des neuen Kunden", JOptionPane.ERROR_MESSAGE);
			else
				JOptionPane.showMessageDialog(this, "Das folgende Feld muss ausgef�llt werden: " + pflichtfelder.toString().substring(1, pflichtfelder.toString().length() - 1), "Fehler beim Anlegen des neuen Kunden", JOptionPane.ERROR_MESSAGE);
	...
	/** �berpr�ft das eingegebene Datum
	 * @return Gibt das Lieferdatum als GregorianDate zur�ck. Oder NULL, wenn das
	 * Datum ung�ltig ist.
	 */
	private GregorianCalendar checkDate() {
		...
		if (lieferTag != lieferDatum.get(Calendar.DATE)) {
			JOptionPane.showMessageDialog(this, "Ung�ltiges Lieferdatum!", "Fehler", 0);
			return null;
		}
	 ...
	 * @param Deleted true: Eine Zeile muss gel�scht werden.
	 * false: Eine Zeile muss hinzugef�gt werden.
	 */
		...
		if (!lieferrabatt.equals("") && lieferrabatt != null) {
			...
			while (!lieferrabattOk && lieferrabatt != null) {
				...
				if (!validate.checkPercent(lieferrabatt)) {
					...
					lieferrabatt = JOptionPane.showInputDialog(this, "Ung�ltiger Lieferrabatt (Format: xx.xx)! Bitte neu eingeben:", lieferrabatt);
		...
		if (!lieferdatum.equals("") && lieferdatum != null) {
			...
			while (!lieferdatumOk && lieferdatum != null) {
				...
				if (!validate.checkYear(lieferdatum)) {
					jOptionPaneConfirm.setWantsInput(true);
					lieferdatum = JOptionPane.showInputDialog(this, "Ung�ltiger Jahr! Bitte neu eingeben:", lieferdatum);
				} else {
					lieferdatumOk = true;
				}
			}
	...
	private void validateDateMonth() {
		...
		if (!lieferdatum.equals("") && lieferdatum != null) {
			//So lange der User keine richtige Eingabe gemacht hat und nicht auf Abbrechen geklickt hat
			while (!lieferdatumOk && lieferdatum != null) {
				//Erlaubt sind nur Eingaben von 1- oder 2-stellige Zahlen || Monatsangabe darf nicht gr��er 12 sein
				if (!validate.checkTwoDigits(lieferdatum) || Integer.parseInt(lieferdatum) > 12) {
					jOptionPaneConfirm.setWantsInput(true);
					lieferdatum = JOptionPane.showInputDialog(this, "Ung�ltiger Monat! Bitte neu eingeben:", lieferdatum);
				} else {
					lieferdatumOk = true;
				}
			}
		...
		if (!lieferdatum.equals("") && lieferdatum != null) {
			...
			while (!lieferdatumOk && lieferdatum != null) {
				...
				if (!validate.checkTwoDigits(lieferdatum) || Integer.parseInt(lieferdatum) > 31) {
					jOptionPaneConfirm.setWantsInput(true);
					lieferdatum = JOptionPane.showInputDialog(this, "Ung�ltiger Tag! Bitte neu eingeben:", lieferdatum);
				} else {
					lieferdatumOk = true;
				}
			}
		...
		if (!skontofrist.equals("") && skontofrist != null) {
			...
			while (!skontofristOk && skontofrist != null) {
				...
				if (!validate.checkZahl(skontofrist)) {
					jOptionPaneConfirm.setWantsInput(true);
					skontofrist = JOptionPane.showInputDialog(this, "Ung�ltige Skontofrist! Bitte neu eingeben:", skontofrist);
				} else {
					skontofristOk = true;
				}
			}
		...
		if (!skonto.equals("") && skonto != null) {
			...
			while (!skontoOk && skonto != null) {
				...
				if (!validate.checkPercent(skonto)) {
					jOptionPaneConfirm.setWantsInput(true);
					skonto = JOptionPane.showInputDialog(this, "Ung�ltiger Skonto-Wert (Format: xx.xx)! Bitte neu eingeben:", skonto);
				} else {
					skontoOk = true;
				}
			}
	...
	/** F�gt eine Position in die Tabelle ein */
	...
	/** Holt Infos �ber den selektierten Article aus der DB */
