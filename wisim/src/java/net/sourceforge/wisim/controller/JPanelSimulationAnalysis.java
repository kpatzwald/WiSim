...
public class JPanelSimulationAnalysis extends javax.swing.JPanel implements Refreshable {
  ...
  public void refreshJTreeEinkauf() {
    actDay++;
    ArrayList<ComponentContract> componentContractList = new ArrayList<>();

    //Liste aller Einzelteilaufträge
    try {
      componentContractList = dao.getEinzelteilauftraege();
    ...
    for (ComponentContract cc : componentContractList) {
      Date lieferDate = cc.getLieferdatum();
      ...
      if (actDateGC.get(5) == lieferDateGC.get(5) && actDateGC.get(2) == lieferDateGC.get(2)) {
        ...
        try {
          lt = dao.getLieferant(cc.getLieferantNr());
        } catch (WiSimDAOException e) {
          wiSimLogger.log("refreshJTreeEinkauf()", e);
        }

        //Neuer Node im JTree
        lieferantNode = new IconNode("Auftrags Nr.: " + cc.getNr() + "    Lieferant: " + lt.getFirma());
        lieferantNode.setIcon(vanIcon);

        ArrayList<ComponentContractItem> ccItems = new ArrayList<>();
        try {
          ccItems = dao.getEinzelteilAuftragsPositionen(cc.getNr());
        } catch (WiSimDAOException e) {
          wiSimLogger.log("refreshJTreeEinkauf()", e);
        }

        int i = 1;
        for (ComponentContractItem ccItem : ccItems) {

          WiSimComponent et = new WiSimComponent();
          try {
            et = dao.getComponent(ccItem.getEtNr());
          } catch (WiSimDAOException e) {
            wiSimLogger.log("refreshJTreeEinkauf()", e);
          }

          boolean status = runController.einzelteilLieferung(et.getNr(), ccItem.getBestellmenge());

          //Neuer Node im JTree 
          if (status) {
            positionNode = new IconNode("Position: " + i + "    Einzelteil: " + et.getName() + "    Menge: " + ccItem.getBestellmenge() + " wurde ins Lager geschafft!");
            positionNode.setIcon(boxIcon);
          } else {
            positionNode = new IconNode("Position: " + i + "    Einzelteil: " + et.getName() + "    Menge: " + ccItem.getBestellmenge() + " überfüllt das Lager!");
  ...
  /**
   * Baut den JTree mit einer Liste aller produzierten Hubs und verbrauchten
   * Einzelteile auf.
   */
    ...
    if (actDay != 1) {
      ...
      if (hubBestandDifferenz > 0) {
        for (String key : stueckliste.keySet()) {
          ...
          try {
            et = dao.getComponent(Integer.parseInt(key));
          } catch (WiSimDAOException e) {
            wiSimLogger.log("refreshJTreeProduktion()", e);
          }

          hubEinzelteilVerbrauch = new IconNode("Einzelteil: " + et.getName() + "    Verbrauch: " + (Integer.parseInt(stueckliste.get(key)) * hubBestandDifferenz + " Stk."));
  ...
  /**
   * Baut den JTree mit einer Liste aller verkauften Hubs für einen Tag auf.
   * Leert außerdem das Lager je nach Anzahl der verkauften Hubs
   */
  public void refreshJTreeVertrieb() {
    ...
    Iterator vertragIt = contracts.iterator();
    while (vertragIt.hasNext()) {
      ...
      if (actDateGC.get(5) == lieferDateGC.get(5) && actDateGC.get(2) == lieferDateGC.get(2)) {
        ...
        try {
          kunde = dao.getCustomer(vertrag.getKundenId());
        } catch (WiSimDAOException e) {
          wiSimLogger.log("refreshJTreeVertrieb()", e);
        }
  ...
  public void initializeCollections() {
    try {
      artikelLagerElemente = dao.getArtikelLagerElement();
      stueckliste = dao.getPartsListForArticle(1);
