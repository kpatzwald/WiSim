 ...
 * jeweiligen Einzelteile aus. Es lassen sich die Anzahl der Arbeiter pro
 * WorkPlace einstellen.
 *
...
public class JPanelWorkPlaceStore extends javax.swing.JPanel implements Refreshable {
  ...
  private ArrayList<WorkPlace> arbeitsplaetze;
  private ArrayList<WorkPlaceStore> arbeitsplatzLager;
  ...
  public JPanelWorkPlaceStore(WiSimMainController wiSimMainController) {
    ...
    arbeitsplatzLager = new ArrayList<>();
    apAnzahl = 0;
    apElemAnzahl = 0;
    isBuilt = false;
  ...
  public void getWorkPlaceStorage() {
    ...
    while (ap_it.hasNext()) {
      ...
      try {
        apLager = dao.getWorkPlaceStore(ap.getNr(), 26, "Eingang");
      } catch (WiSimDAOException e) {
        wiSimLogger.log("getArbeitsplatzLager()", e);
      }
      ...
      try {
        apLager = dao.getWorkPlaceStore(ap.getNr(), 26, "Ausgang");
      } catch (WiSimDAOException e) {
        wiSimLogger.log("getArbeitsplatzLager()", e);
      }
  ...
  private void getWorkPlaceStorageElements() {
    WorkPlace ap = arbeitsplaetze.get(jTableArbeitsplaetze.getSelectedRow());
  ...
  public void buildWorkPlaceStoreElementsTable() {
    ...
    while (arbeitsplatzLager_it.hasNext()) {
      ...
      try {
        et = dao.getComponent(apLager.getEinzelteilNr());
      } catch (WiSimDAOException e) {
